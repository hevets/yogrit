...
Yogrit.prototype.startRunner = function() {
  ...
  async.eachSeries(this.updateQ, function(file, callback) {
    if(typeof file.query !== 'function') this.migration(file, callback);
    else this.migrationSeries(file, callback);
  }, function(err) {
...
Yogrit.prototype.migration = function(file, callback) {
  ...
  async.series({
    ...
    console.log('Records updated:   ', results.up);
...
Yogrit.prototype.migrationSeries = function(file, callback) {
  ...
  async.whilst(
    ...
    function(callback) {
      self.migrationEach(file, callback);
    },
    function(err) {
